...
export const playingListSlice = createSlice({
    ...
    reducers: {
        ...
        removeFromListInAddOrder: (state, action: PayloadAction<SongType>) => {
            const newList = state.listInAddOrder.filter(item => item != action.payload)
        ...
        playNext: (state) => {
            const nowIndex = state.listInPlayOrder.findIndex(item => item.id === state.playingSong.id)
...
export const { addTolistInAddOrder, removeFromListInAddOrder, changePlayingSong, playNext, playPrev } = playingListSlice.actions
